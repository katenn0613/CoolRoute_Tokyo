...
- Road Graph: <https://katenn0613.github.io/CoolRoute_Tokyo/data/graph_tokyo23.bin.gz>
- Runtime Metadata: <https://katenn0613.github.io/CoolRoute_Tokyo/data/graph_tokyo23_runtime_metadata.json>
...
- Building Shade Metadata: <https://katenn0613.github.io/CoolRoute_Tokyo/data/shade_metadata_tokyo23.json>
- Heat / Shade MVT: `public/data/tiles/`

Production Browser 只读取这些 Binary、JSON、GeoJSON 和 MVT 静态资源。GraphML、Shapefile、CSV、GeoPackage 及其他 Raw GIS 只用于离线预处理，不进入 Pages Runtime。
...
- **Tokyo23 诊断结论：** 旧 JSON 已退出 Runtime；当前默认 Production 使用后述的独立 Binary/MVT 产物

### Tokyo23 Route A Production 运行时

- **入口：** 普通 Pages URL 默认加载；`?dataset=tokyo-core5` 可显式使用回退数据，初始化失败也会自动回退 Core5
...
- **状态：** `ready` Production；完整机器可读说明见 `public/data/graph_tokyo23_runtime_metadata.json`
...
- **当前状态：** `ready`
- **M11：** 已按相同语义白名单与 15m Buffer 规则扩展到 Tokyo23 Binary Graph，metadata 为 `public/data/environment_metadata_tokyo23.json`。
...
- **当前状态：** `ready`
- **M11：** `public/data/drinking_stations_tokyo23.geojson` 发布东京23区范围内有效官方点；全部有效点继续参与最近距离计算。
