# Tokyo23 Route A Production 整合设计

## 目标

在当前 `main` 的 React/UI、Core5 Production、三语 README 和 Pages Workflow 基础上，移植 `dev/route-a-perf` 的紧凑二进制路网、Web Worker Routing 与 Heat/Shade MVT 图层。东京23区作为默认 Dataset，Core5 继续作为可恢复 fallback；不提交约 288MB 的全量源 JSON。
...
- 初始化失败时 UI 显示明确错误，并允许切换/回退 Core5；不把 Shade 缺失伪装为 0。
...
- Tokyo23 10组同组件 OD 成功；跨组件 OD 返回明确错误而非崩溃。
